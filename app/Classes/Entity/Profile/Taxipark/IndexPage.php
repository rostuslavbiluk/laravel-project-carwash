...
namespace App\Classes\Entity\Profile\Taxipark;
...
class IndexPage
{
    ...
    public static function index()
    {
        ...
        $returnResult = [
            'SHOW_BLOCK_ENTITY' => 'Y',
            'ENTITY' => false,
            'SHOW_BLOCK_NEW_ORDERS' => 'N',
            'NEW_ORDERS' => [],
            'SHOW_BLOCK_COMPLETE_ORDERS' => 'Y',
            'COMPLETE_ORDERS' => [],
            'SHOW_BLOCK_STATISTICS' => 'Y',
            'STATISTICS' => [],
        ];
        $organizations = Organizations::where('user_id', $currentUser->id)->first();
        if ($organizations) {
            $organizations = $organizations->toArray();
            $returnResult['ENTITY'] = $organizations;

            if (isset($returnResult['ENTITY']['id'])) {

                $orders = Order::where('entity_id', $returnResult['ENTITY']['id'])->get();
